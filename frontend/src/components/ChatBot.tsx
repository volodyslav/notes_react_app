...
const ChatBot = () => {
  ...
  return (
    <div className="relative">
        <div className={`${theme === "dark" ? "bg-secondary text-success" : "bg-success text-secondary"} ${showInput ? "visible chat-bot" : "chat-bot-close"} chat w-[250px] md:min-w-[500px] h-[400px] md:h-[400px] p-6 absolute right-0 bottom-24 z-50 rounded-2xl  border-2 border-yellow-800`}>
            {responceShow && <textarea readOnly  className=" resize-none text-black min-w-full h-[200px] p-2 text-xl ">{responceShow }</textarea>}
            
            <div className="flex space-x-2 items-center justify-center ">
                {!loading && <textarea  maxLength={100} minLength={10} value={text} onChange={(e) => setText(e.target.value)} placeholder="Ask question about your notes" className={`shadow  text-xl  duration-700 ease-in-out text-black appearance-none my-4 p-2 border-2 resize-none rounded max-w-fit mb-3  leading-tight focus:outline-none ${theme === "dark" ? "bg-success  border-success" : "bg-primary  border-secondary"}`} />}
